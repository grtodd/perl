...
S_is_cur_LC_category_utf8(pTHX_ int category)
{
     ...
     * otherwise. 'category' may not be LC_ALL. */
    ...
    { /* Next try nl_langinfo if available */
        ...
        if (category != LC_CTYPE) { /* nl_langinfo works only on LC_CTYPE */
            ...
            /* Get both the current LC_CTYPE and desired category's locales */
            save_ctype_locale = stdize_locale(savepv(setlocale(LC_CTYPE, NULL)));
            if (! save_ctype_locale) {
                goto cant_use_nllanginfo;
            }

            /* If LC_CTYPE and the desired category are the same, this means
             * that finding the value for LC_CTYPE is the same as finding the
             * value for the desired category.  Otherwise, switch LC_CTYPE to the
             * desired category's locale */
